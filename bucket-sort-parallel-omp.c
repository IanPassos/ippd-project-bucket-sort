...
void insertionSort(int bucket[], int pos)
{
    for (int i = 1; i < pos; ++i)
    {
        int key = bucket[i];
...
void bucketSort(int A[], int n, int limit)
{
    int i, j; // Contadores
    ...
    // 1 - Criando N Buckets
    int **b = (int **) malloc(n * sizeof(int *)); // Número de buckets
...
#pragma omp parallel
    ...
    #pragma omp for private(i, bi)
    for (i = my_id*(n/num_threads); i < (my_id + 1)*(n/num_threads); i++)
    {
        bi = (int)((float)n * ((float)A[i] / (float)limit));
        b[bi][pos[bi]] = A[i];
        pos[bi]++;
    }
...
int main()
{
    int *A; // Ponteiro para o vetor de elementos
    int i; // Contador
